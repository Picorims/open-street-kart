...
@export var procedural_data_holder: Node3D
...
func init(mode: GameMode, speed: SpeedMode, cars_count: int):
	if not is_multiplayer_authority():
		print("Client tried to init track!")
		return
	else:
		print("Server initializing track...")
		return
	_mode = mode
	_speed = speed

...
var _game_mode: TrackStateModel.GameMode
...
func _init(max_speed: float, game_mode: TrackStateModel.GameMode) -> void:
	...
	for i in range(SLOTS_COUNT):
		...
		if (i == 0 and _game_mode == TrackStateModel.GameMode.VERSUS):
			_slots[i].set_type(SlotItem.EMPTY)
			_slots[i].lifetime_s = MIN_REFILL_TIME_SECONDS
		if _game_mode == TrackStateModel.GameMode.AGAINST_CLOCK:
...
func tick(delta: float, distance_to_first: float, use_item: bool, rank: int) -> SlotItem:
	...
	var i: int = 0
	while i < SLOTS_COUNT:
		if _game_mode == TrackStateModel.GameMode.VERSUS:
